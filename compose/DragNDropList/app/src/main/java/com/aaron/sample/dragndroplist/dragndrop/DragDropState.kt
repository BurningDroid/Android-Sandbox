...
class DragDropState internal constructor(
    ...
    fun checkForOverScroll(): Float {
        return initiallyDraggedElement?.let {
            ...
            return@let when {
                draggedDistance > 0 -> (endOffset - state.layoutInfo.viewportEndOffset + 250f).takeIf { diff -> diff > 0 }
                draggedDistance < 0 -> (startOffset - state.layoutInfo.viewportStartOffset - 250f).takeIf { diff -> diff < 0 }
                else -> null
            }
        } ?: 0f
    }
